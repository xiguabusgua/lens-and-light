...
export function initializeDatabase(): void {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_album_works_work_id ON album_works(work_id);
  `);
  ...
  if (worksCount.count === 0) {
    const insert = db.prepare(`
      INSERT INTO works (title, category, image_url, description, story, camera, lens, aperture, shutter, iso, location, date, featured, sort_order, status, tags) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    const insertMany = db.transaction((works: any[]) => {
  ...
  if (catCount.count === 0) {
    ...
    categorySeed.forEach(cat => insertCat.run(...cat));
  }
  ...
  if (tagCount.count === 0) {
    ...
    const tagSeed = [
      ...
      ['长曝光', 'long-exposure', 1],
      ['产品', 'product', 1],
      ['纪实', 'documentary-tag', 1],
      ['风光', 'landscape-tag', 1],
      ['人文', 'humanities', 1]
    ];

    tagSeed.forEach(tag => insertTag.run(...tag));

    const linkWorkTag = db.prepare(`
      INSERT OR IGNORE INTO work_tags (work_id, tag_id) VALUES (?, ?)
    `);

    const linkMany = db.transaction((links: [number, number][]) => {
      for (const [workId, tagId] of links) {
        linkWorkTag.run(workId, tagId);
      }
    });

    const workTagLinks: [number, number][] = [
      [1, 1], [1, 2], [1, 7],
      [2, 4], [2, 10], [2, 13],
      [3, 5], [3, 6], [3, 8],
      [4, 3], [4, 2], [4, 14],
  ...
  if (albumCount.count === 0) {
    ...
    const albumSeed = [
      ['四季风光', 'seasons', '记录春夏秋冬四季轮回中的绝美风光，从春日花开到冬雪皑皑', 'https://picsum.photos/seed/photo4/1920/1280', 'active', 1],
      ['人间烟火', 'human-life', '街头巷尾的人间百态，每一个微笑和皱纹背后的故事', 'https://picsum.photos/seed/photo3/1920/1280', 'active', 2],
      ['光影肖像', 'portraits', '用光影讲述人物的故事，捕捉灵魂深处的真实', 'https://picsum.photos/seed/photo2/1280/1920', 'active', 3]
    ];

    albumSeed.forEach(album => insertAlbum.run(...album));

    const insertAlbumWork = db.prepare(`
      INSERT INTO album_works (album_id, work_id, sort_order)
      VALUES (?, ?, ?)
    `);

    const albumWorkLinks = [
      [1, 1, 1], [1, 4, 2], [1, 9, 3],
      [2, 3, 1], [2, 7, 2], [2, 10, 3],
      [3, 2, 1], [3, 6, 2], [3, 5, 3]
    ];

    albumWorkLinks.forEach(link => insertAlbumWork.run(...link));
  }

  if (worksCount.count === 0 || catCount.count === 0 || tagCount.count === 0) {
